...
class RecentTradesComponent extends React.Component {
  ...
  getTrades(trades) {
    const priceFixed = this.props.currentMarket
      ? this.props.currentMarket.bid_precision
      : 0
    const amountFixed = this.props.currentMarket
      ? this.props.currentMarket.ask_precision
      : 0
    const renderRow = item => {
      const { id, created_at, taker_type, price, volume } = item;      
      let formatPrice  = price.slice(0, 10);
      for(let i = price.length; i <= 9; i++){
        formatPrice = formatPrice + '0';
      }
      return [
        React.createElement(
          'span',
          { style: { color: setTradeColor(taker_type).color }, key: id },
          React.createElement('span', {} ,formatPrice),
        ),
        React.createElement(
          'span',
          { style: { color: setTradeColor(taker_type).color }, key: id },
